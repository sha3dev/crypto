...
function isNotFoundError(output) {
  ...
  return (
    lower.includes("e404") || lower.includes("404 not found") || lower.includes("no match found")
  );
}
...
async function main() {
  ...
  if (exists) {
    ...
    console.log(
      `Version ${packageJson.name}@${packageJson.version} already existed. Bumped to ${nextVersion}.`
    );
  } else {
